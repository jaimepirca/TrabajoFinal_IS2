package com.servlet;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import com.bean.PacienteBean;
import com.bean.ReporteBean;
import com.jdbc.RegistrosDAO;
import com.jdbc.RegistrosIF;

/**
 * Servlet implementation class Cancelarmed
 */
public class Cancelarmed extends HttpServlet {
	private static final long serialVersionUID = 1L;
   
	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
		
		HttpSession sesion = request.getSession(true);
		
		List<ReporteBean> l = new ArrayList<ReporteBean>();
		
		PacienteBean p = (PacienteBean)sesion.getAttribute("PACIENTE");
		
		RegistrosIF dao = new RegistrosDAO();
		
		l = dao.getMedicamentoEnEspera(p.getCodigo());
		
		 sesion.setAttribute("MedicamentoPend", l);
		    
		    
		    RequestDispatcher rd = request.getRequestDispatcher("/cancelarmed.jsp");
			rd.forward(request,response);
	}

	
}
